...
export async function numberPrompt(message: string, defaultValue: number): Promise<number> {
  const value = await number({
    message,
    default: defaultValue,
    validate: (candidate) => (typeof candidate === "number" && Number.isFinite(candidate) ? true : "Please enter a valid number")
  });
